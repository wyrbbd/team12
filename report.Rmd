...
Sexual_assault %>%
  pivot_longer(cols = '2010':'2019',  
                names_to = "Year",            
                values_to = "Count") -> SA_longer

SA_longer %>%
  pivot_wider(id_cols = c(age, Year, Count), names_from = age, values_from = Count) %>%
  mutate(Year = as_factor(Year)) %>%
  mutate(class = "Sexual_assault") -> SA_wider

SA_longer %>%
  group_by(age) %>%
  summarise(sum = sum(Count)) %>%
  mutate(class = "Sexual_assault") %>%
...
Armed_robbery %>%
  pivot_longer(cols = '2010':'2019',  
                names_to = "Year",            
                values_to = "Count") -> AR_longer

AR_longer %>%
  pivot_wider(id_cols = c(age, Year, Count), names_from = age, values_from = Count) %>%
  mutate(Year = as_factor(Year)) %>%
  mutate(class = "Armed_robbery") -> AR_wider


AR_longer %>%
  group_by(age) %>%
  summarise(sum = sum(Count)) %>%
  mutate(class = "Armed_robbery") %>%
...
Unarmed_robbery %>%
  pivot_longer(cols = '2010':'2019',  
                names_to = "Year",            
                values_to = "Count") -> UR_longer

UR_longer %>%
  pivot_wider(id_cols = c(age, Year, Count), names_from = age, values_from = Count) %>%
  mutate(Year = as_factor(Year)) %>%
  mutate(class = "Unarmed_robbery") -> UR_wider

UR_longer %>%
  group_by(age) %>%
  summarise(sum = sum(Count)) %>%
  mutate(class = "Unarmed_robbery") %>%
  filter(age != "Total") -> UR_age
```

```{r Unarmed, echo = FALSE, message = FALSE, warning = FALSE}
Unarmed_robbery %>%
  pivot_longer(cols = '2010':'2019',  
                names_to = "Year",            
                values_to = "Count") -> UR_longer
...
Blackmail_extortion %>%
  pivot_longer(cols = '2010':'2019',  
                names_to = "Year",            
                values_to = "Count") -> BE_longer

BE_longer %>%
  pivot_wider(id_cols = c(age, Year, Count), names_from = age, values_from = Count) %>%
  mutate(Year = as_factor(Year)) %>%
  mutate(class = "Blackmail_extortion") -> BE_wider

BE_longer %>%
  group_by(age) %>%
  summarise(sum = sum(Count)) %>%
  mutate(class = "Blackmail_extortion") %>%
...
data %>%
  ggplot() + geom_col(aes(x= class, y = sum, fill = age), position = "fill")
